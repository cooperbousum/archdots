...
set_second_conf() {
  ...
  hyprctl keyword animation layersOut, 1, 10, easeOutQuint, slide right && killall waybar || true && hyprctl keyword animation layersIn, 1, 25, easeOutQuint, slide left && hyprctl dispatch exec waybar --quiet
}

set_first_conf() {
  cp $waybar_path/firstconf.jsonc $waybar_path/config.jsonc
  cp $waybar_path/firststyle.css $waybar_path/style.css
  hyprctl keyword animation layersOut, 1, 10, easeOutQuint, slide right && killall waybar || true && hyprctl keyword animation layersIn, 1, 5, easeOutQuint, slide left && hyprctl dispatch exec waybar --quiet
}
...
reload_conf() {
  ...
  if [ $windows -eq 0 ]; then
    set_second_conf
  elif [ $windows -gt 0 ] && [ $prevwindows -eq 0 ]; then
    set_first_conf
  fi
  prevwindows=$windows
}
